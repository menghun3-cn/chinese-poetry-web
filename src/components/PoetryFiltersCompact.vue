<template>
  <div class="filter-root">
    ...
    <!-- 热门标签 + 常读作者 (collapsible, default expanded) -->
    <div class="quick-bar">
      <button class="quick-toggle" @click="showQuick = !showQuick">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="size-3"><polyline points="6 9 12 15 18 9"/></svg>
        <span class="quick-label">快捷筛选</span>
        <span class="quick-hint" v-if="store.frequentAuthors.length || store.hotTags.length">
          常读作者 ({{ store.frequentAuthors.length }}) · 热门标签 ({{ store.hotTags.length }})
        </span>
      </button>
      <Transition name="quick-slide">
        <div v-if="showQuick" class="quick-content">
          <!-- Frequent Authors -->
          <div v-if="store.frequentAuthors.length" class="quick-section">
            <span class="quick-section-label">
            ...
            <div class="quick-chips">
              <button v-for="author in store.frequentAuthors" :key="author.name"
                ...
                {{ author.name }}
                <span class="quick-chip-count">{{ author.count }}</span>
              </button>
            </div>
          </div>
          <!-- Hot Tags -->
          ...
          <div v-if="!store.frequentAuthors.length && !store.hotTags.length" class="quick-empty">
            阅读诗词后，常读作者和热门标签将自动出现在这里
          </div>
        </div>
      </Transition>
    </div>
      ...
      <div v-if="showTagPanel" class="tag-panel-overlay" @click.self="showTagPanel = false">
        <div class="tag-panel-card">
          <div class="tag-panel-header">
            <span class="tag-panel-title">选择标签</span>
            <button class="tag-panel-close" @click="showTagPanel = false">关闭</button>
          </div>
...
const hasActiveFilters = computed(() =>
  store.filters.keyword !== '' || store.filters.collectionId !== 'all' || store.filters.tags.length > 0 || store.filters.author !== ''
)

function onSearch(e: Event) {
  store.updateFilters({ keyword: (e.target as HTMLInputElement).value })
}
...
function toggleTag(tag: string) {
  ...
  store.updateFilters({ tags: current })
}
...
function clearHistory() {
  ...
  store.frequentAuthors.length = 0
  store.hotTags.length = 0
}

watch(() => store.filters, () => {
  // Auto-collapse quick bar when filter changes to keep UI clean
}, { deep: true })
</script>
...
/* Filter bar */
.filter-bar { display:flex; align-items:center; gap:0.375rem; padding:0.5rem 0.75rem; flex-wrap:wrap; }
@media (min-width:640px){ .filter-bar{ flex-wrap:nowrap; } }

.filter-search-wrap { position:relative; flex:1; min-width:8rem; }
...
.filter-search-input { width:100%; padding:0.375rem 0.5rem 0.375rem 1.75rem; font-size:0.8125rem; border-radius:0.5rem; border:1px solid var(--color-border); background:var(--color-page); color:var(--color-text); outline:none; transition:border-color 0.15s ease; }
...
/* Quick bar - 常读作者 / 热门标签 */
.quick-bar { border-top:1px solid var(--color-border); }
.quick-toggle { display:flex; align-items:center; gap:0.375rem; width:100%; padding:0.25rem 0.75rem; font-size:0.6875rem; color:var(--color-text-muted); background:none; border:none; cursor:pointer; transition:color 0.15s ease; }
.quick-toggle:hover { color:var(--color-text); }
.quick-label { font-weight:500; }
.quick-hint { margin-left:auto; font-size:0.625rem; opacity:0.6; }

.quick-content { padding:0.25rem 0.75rem 0.5rem; display:flex; flex-direction:column; gap:0.375rem; }
...
.quick-chip { display:inline-flex; align-items:center; gap:0.25rem; padding:0.125rem 0.5rem; font-size:0.6875rem; font-weight:500; border-radius:9999px; border:1px solid var(--color-border); background:var(--color-surface); color:var(--color-text-secondary); transition:all 0.15s ease; cursor:pointer; white-space:nowrap; }
...
.quick-empty { font-size:0.75rem; color:var(--color-text-muted); padding:0.375rem 0; text-align:center; }
...
/* Transitions */
.quick-slide-enter-active, .quick-slide-leave-active { transition:all 0.2s ease; }
.quick-slide-enter-from, .quick-slide-leave-to { max-height:0; opacity:0; padding-top:0; padding-bottom:0; }
.quick-slide-enter-to, .quick-slide-leave-from { max-height:20rem; }
